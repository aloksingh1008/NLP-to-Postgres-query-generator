...
for i in range(0, len(all_ids), batch_size):
    ...
    SELECT 
        ...
        c.is_nullable,
        case when c.column_name='state' then 'status' when f.field_name is null then c.column_name  else f.field_name end as field_name 
    FROM cols c
    LEFT JOIN form_fields f
        ...
        AND (c.table_name = 'table' || f.parent_id OR c.table_name = 'table' || f.relation);
    """
...
# ---------- Write to CSV ----------
with open("form_table_schema.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["table_name","column_name", "data_type", "character_maximum_length", "is_nullable", "field_name"])
    writer.writerows(results)

print("✅ Data saved to form_table_schema.csv")
